...
export default function ZugangPage() {
  ...
  // Solange die Seite in Überarbeitung ist, sehen sie nur Admin-Konten.
  if (!ZUGANG_OEFFENTLICH && status !== "admin") {
    return (
      <div className="max-w-lg mx-auto px-5 py-16">
        <div className="bg-white rounded-xl shadow-sm p-8 text-center">
          {status === "pruefe" ? (
            <p className="text-sm text-bbw-muted">Wird geprüft…</p>
          ) : (
            <>
              ...
              <p className="text-sm text-bbw-muted">
                Das Zugangs- und Berechtigungskonzept wird derzeit überarbeitet
                und ist noch nicht aufgeschaltet. Es wird veröffentlicht, sobald
  ...
  return (
    <div className="max-w-6xl mx-auto px-5 py-8">
      {!ZUGANG_OEFFENTLICH && (
        <div className="mb-6 px-4 py-3 rounded-lg bg-amber-50 border border-amber-200 text-xs text-amber-900 no-print">
          <strong>Noch nicht aufgeschaltet.</strong> Diese Seite ist in
          Überarbeitung und nur für Konten der Administration sichtbar. Zum
          Veröffentlichen <code>ZUGANG_OEFFENTLICH</code> in{" "}
          <code>src/lib/feature-flags.ts</code> auf <code>true</code> setzen.
        </div>
      )}
